...
void Lab6::Init()
{
...

    {
        ...
        for (angle = 0.0f; angle <= (2.0f * 3.14159265359); angle += 0.1)
        {
            float x = 2 * sin(angle);
            float y = 2 * cos(angle);
            vertices1.push_back(VertexFormat(glm::vec3(x, -1, y), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)));
            //if (i > 0) {
        ...
        float x = 2 * sin(angle);
        float y = 2 * cos(angle);
        vertices1.push_back(VertexFormat(glm::vec3(x, -1, y)));
        //if (i > 0) {
        indices1.push_back(-1);
        indices1.push_back(i);
        indices1.push_back(i + 1);

        CreateMesh("con", vertices1, indices1);
    }
...
void Lab6::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(1.f, 2, 2));
        //modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 0, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.65f));
        RenderSimpleMesh(meshes["con"], shaders["LabShader"], modelMatrix);
    }
}
