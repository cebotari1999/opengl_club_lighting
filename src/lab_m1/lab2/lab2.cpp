...
void Lab2::Init()
{
...

    {
        ...
        for (angle = 0.0f; angle <= (2.0f * 3.14159265359); angle += 0.1)
        {
            float x = 2 * sin(angle);
            float y = 2 * cos(angle);
            vertices3.push_back(VertexFormat(glm::vec3(x, -2, y), glm::vec3(0, 1, 0)));
            //if (i > 0) {
        ...
        // Actually create the mesh from the data
        CreateMesh("cube_B", vertices3, indices3);
    }
...
void Lab2::Update(float deltaTimeSeconds)
{
   ...
   // glCullFace(cullFace);
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(1.0f, 1.0f, 0), glm::vec3(0.5f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(2.0f, 0.5f, 0), glm::vec3(0.5f));


    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-3.0f, 0.5f, 0), glm::vec3(0.7f));
